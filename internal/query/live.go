...
// LiveVisitorPoint is one visitor currently active on the site.
...
func LiveVisitorsHandler(db *gorm.DB) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		results, err := GetLiveVisitors(db)
		if err != nil {
...
// GetLiveVisitors returns everyone currently active on the site (any
// event within liveWindow), sorted by how long they've been
// continuously active, longest first.
func GetLiveVisitors(db *gorm.DB) ([]LiveVisitorPoint, error) {
	now := time.Now().UTC()
